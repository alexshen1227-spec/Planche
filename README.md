...
- **Coach** — an on-device optimizer that treats "how should today's sets be shaped?" as a multi-armed bandit over five strategies (balanced, high volume, high intensity, short rests, technique). Reward is how much your key hold actually moved by the next session, normalized against the unlock bar; selection is UCB1, so untested strategies get explored and the fastest one gets exploited. All of it is derived from your logged history — see `src/lib/coach.ts`.
