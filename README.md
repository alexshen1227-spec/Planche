...
- **Coach** — an on-device optimizer aimed at one goal: reaching the next planche step as fast as safely possible.
  ...
  - *Spots bad measurements.* When session-to-session holds swing more than ~22%, it freezes targets and says so instead of chasing noise; single wild values are flagged as outliers rather than trusted as a new baseline.
  ...
  - *Asks.* A periodic pre-session check-in (joints, energy) that re-plans the current session immediately.
